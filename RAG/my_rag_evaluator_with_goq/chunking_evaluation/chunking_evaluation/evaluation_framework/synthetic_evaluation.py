...
class SyntheticEvaluation(BaseEvaluation):
    def __init__(self, corpora_paths: List[str], queries_csv_path: str, chroma_db_path:str = None, openai_api_key=None, model_provider:str="openai", embedding_provider:str="openai"):
    ...
    def _extract_question_and_approx_references(self, corpus, document_length=4000, prev_questions=[]):
        ...
        if self.model_provider == "openai":
            ...
            self.client = Groq(api_key=self.openai_api_key)
            completion = self.client.chat.completions.create(
                model="llama-3.3-70b-versatile",
                response_format={ "type": "json_object" },
                max_tokens=600,
